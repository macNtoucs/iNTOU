...
void jc_int2bin(
                    Word16* value,             /* input : decimal value */
                    char *bitstream           /* output: bitstream  */
)
{
    int bits = 0;
    ...
    {
        for(int j = bitsno[i] - 1; j >= 0 ; j--)
        {
            if(value[i] & ((Word16)1 << j))
                bitstream[bits/8] |= ((char)1 << (7 - (bits%8)));
            bits ++;
        }
    }
}
...
void jc_bin2int(
                char bits[],          /* input : serial bits (80)                       */
                Word16   prm[]            /* output: decoded parameters (11 parameters)     */
)
{
    int bits_seted = 0;
    for (int i = 0; i < PRM_SIZE; i++) {
        prm[i] = 0;
        for(int j = 0; j < bitsno[i]; j++) {
            prm[i] <<= 1;
            if(bits[bits_seted/8] & 0x80) prm[i] += 1;
            bits[bits_seted/8] <<= 1;
            bits_seted ++;
        }
    }
